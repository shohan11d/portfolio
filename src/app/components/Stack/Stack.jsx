...
{ stack && <span style={{outlineColor: stack && stack?.color}} className="xs:text-[8px] xs:leading-[8px] xs:px-2  md:text-[11px]  absolute transition-all top-[14px] right-[17px] text-xs outline-[1px] outline-black bg-black text-white dark:outline-white outline dark:bg-white dark:text-black outline-offset-2 rounded-full px-5  leading-[10px]">{stack?.exp} {Number(stack?.exp) > 1 ? "years" : "year"} <span>{!stack && "coding"} {"of experience"}</span></span>}       
 <h2 className="mt-3 md:m-0 leading-snug md:text-center xs:text-wrap text-nowrap dark:mix-blend-difference relative">
        ...
        className="bg-center bg-no-repeat bg-contain rounded-3xl border-black/20  dark:border-[hsl(0,0%,9%)] border-[1px]  saturate-0 bg-origin-content transition-all  betterhover:hover:saturate-100 md:saturate-100 flex-grow-0  flex items-center justify-items-center p-6 sm:p-3 relative mb-5">
        <p style={{ color: stack?.color }} className={`absolute top-full left-1/2 transform -translate-x-1/2`}>{skills_imgs[skill].name}</p>
        </div>
        ))}
</div>
   </section>
