...
<div className="grid grid-rows-3 gap-2 grid-cols-7 md:grid-cols-4 xs:grid-cols-3 xs:grid-rows-6 md:grid-rows-5 md:px-2 my-[80px]">
...
{ stack && <span style={{ outlineColor: stack.color }} className="xs:text-[8px] xs:leading-[8px] xs:px-2 md:text-[11px] absolute transition-all top-[14px] right-[17px] text-xs outline-[1px] bg-black/5 text-comment-grey dark:bg-white/5 dark:text-comment-grey outline-offset-2 rounded-full px-5 leading-[10px]">{stack?.exp} {Number(stack?.exp) > 1 ? "years" : "year"} <span>{"of experience"}</span></span>}
 <h2 className="mt-3 md:m-0 leading-snug md:text-center xs:text-wrap text-nowrap text-comment-grey relative">
        {"Solving real problems with"}<br/>{!stack && <span className="animate-text-slide">{"an constantly expanding stack"}</span>}
        ...
        const skillData = skills_imgs[skill];
        const isActive = stack?.name === skillData.name;

        return (
        <div key={skill} style={{backgroundImage: `url(${skillData.image})`, borderColor: isActive ? skillData.color : undefined}} onMouseLeave={() => handleStackLeave()} onMouseOver={() => handleStackHover(skillData)} 
         className={`bg-center bg-no-repeat bg-contain rounded-3xl border-[1px] bg-origin-content transition-all flex-grow-0 flex items-center justify-items-center p-6 sm:p-3 relative mb-5 ${
          isActive
            ? 'saturate-100'
            : 'border-black/20 dark:border-[hsl(0,0%,9%)] saturate-0'
        }`}>
        <p style={{ color: isActive ? skillData.color : undefined }} className={`absolute top-full left-1/2 transform -translate-x-1/2 transition-colors ${!isActive && 'text-comment-grey'}`}>{skillData.name}</p>
        </div>
        );
        })}
</div>
